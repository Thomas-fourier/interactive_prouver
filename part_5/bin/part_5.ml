...
let rec red_aux (env: context) = function
  | Type -> (Type, false)
  | Var v -> (try get_value_from_context v env, true with Type_error -> (Var v, false))
  | App (Abs (x, _, f), y) -> (subst x y f, true)
  | App (x, y) ->
      let x_reduced, x_bool = red_aux env x and
      y_reduced, y_bool = red_aux env y in
      App (x_reduced, y_reduced), x_bool || y_bool
  | Abs (x, y, z) ->
      let a, b = red_aux ((x, (y, None))::env) z in
      (Abs (x, a, z), b)
  | Pi (x, y, z) ->
      let y_op, y_ch = red_aux ((x, (Type, None))::env) y and
          z_op, z_ch = red_aux ((x, (y, None))::env) z in
...
let rec normalize env t = match red_aux env t with
...
let rec alpha = function
  ...
  | Abs (xvar1, ty1, f1) -> (
      ...
      | Abs (xvar2, ty2, f2) -> let xvar = fresh_var () in
...
let rec infer env = function
  ...
  | App (f, x) -> (
      ...
      | Pi (type_a, a, b) when conv env x a -> infer ((type_a, (a, None)) :: env) b
      | _ -> raise Type_error)
